    ...
    public class HardHeat {
        ...
        public void SaveToVTK(string path) {
            ...
            using (var fs = new StreamWriter(File.Open(path, FileMode.OpenOrCreate))) {
                ...
                for (int i = 0; i < Mesh.Nodes.Count; i++) {
                    fs.Write($"{ Mesh.Nodes[i].X.ToString().Replace(",", ".") } { Mesh.Nodes[i].Y.ToString().Replace(",", ".") } 0.0 ");
                    if (i + 1 % 8 == 0) {
                        fs.WriteLine();
                    }
                }
                ...
                for (int i = 0; i < Mesh.Cells.Count; i++) {
                    //if (last != $"3 { Mesh.Cells[i].Node[0].Id } { Mesh.Cells[i].Node[1].Id } { Mesh.Cells[i].Node[2].Id }") {
                        last = $"3 { Mesh.Cells[i].Node[0].Id - 1 } { Mesh.Cells[i].Node[1].Id - 1 } { Mesh.Cells[i].Node[2].Id - 1 }";
                        fs.WriteLine(last);
                    //}
                }
                fs.WriteLine();
                ...
                fs.WriteLine("LOOKUP_TABLE default");
                for (int i = 0; i < Mesh.Cells.Count; i++) {
                    fs.Write($"{ string.Format("{0:0.000000000000000000}", Mesh.Cells[i].Param.T).Replace(",", ".") } ");
